...
function JobCard({ job, compid }) {
  const navigate = useNavigate();
  function clickHandler(id) {
    navigate("/details", { state: { data: job, compid: compid } });
  }
    ...
    <div className="job_maindiv">
      {job !== undefined && (
        <>
            ...
            <Col md={8}>
              <div className="job_bluebar">
                <p className="job_bluebar_text">
                  ₹{job.minsalary} - ₹{job.maxsalary} a month • Remote •
                  Internship
                </p>
              </div>
            </Col>
          ...
          <button
            type="button"
            onClick={() => clickHandler(job.jobid)}
            className="job_apply_button"
          >
            Apply Now
          </button>
        </>
